...
#include "RCInput.h"
#if RECEIVER_TYPE == ST24_RECEIVER
...
void YUNEECRCInputST24::init(void* machtnichts) {
	/* initiate uartD for st24 */
	_st24_init(hal.uartD);
	...
	/* we check the uart ring buffer per 1ms */
 ...
 *
 * Open the uart for receiving ST24 frames and configure it appropriately
 *
 */
...
void YUNEECRCInputST24::_st24_input(void) {
	...
	for (int i = 0; i < nbytes; i++) {
		...
		if (!st24_decode(byte, &_rssi, &_rx_count, &_channel_count, _channels, ST24_RC_INPUT_CHANNEL)) {
			// return once we lost RF frame
			if (_rx_count > 0)
				return;
			// finally we got packet
